...
static const struct option opts[] = {{"count", no_argument, NULL, 'c'},
...
static void rsu_client_usage(void)
{
	...
	printf("%-32s  %s", "-z|--size slot_num", "get the slot size in bytes\n");
	printf("%-32s  %s", "-p|--priority slot_num", "get the priority of the selected slot\n");
	printf("%-32s  %s", "-E|--enable slot_num",
	       "set the selected slot as the highest priority\n");
	printf("%-32s  %s", "-D|--disable slot_num", "disable selected slot but to not erase it\n");
	...
	printf("%-32s  %s", "-e|--erase slot_num", "erase app image from the selected slot\n");
	...
	printf("%-32s  %s", "-C|--clear-error-status", "clear errors from the log\n");
	printf("%-32s  %s", "-Z|--reset-retry-counter", "reset current retry counter\n");
	printf("%-32s  %s", "-m|--display-dcmf-version", "print DCMF version\n");
	printf("%-32s  %s", "-y|--display-dcmf-status", "print DCMF status\n");
	printf("%-32s  %s", "-x|--display-max-retry", "print max_retry parameter\n");
	printf("%-32s  %s",
	...
	printf("%-32s  %s", "-k|--check-running-factory",
	       "check if currently running the factory image\n");
...
static int rsu_client_display_dcmf_version(void)
{
	...
	for (i = 0; i < 4; i++) {
		printf("DCMF%d version = %d.%d.%d\n", i, (int)DCMF_VERSION_MAJOR(versions[i]),
		       (int)DCMF_VERSION_MINOR(versions[i]), (int)DCMF_VERSION_UPDATE(versions[i]));
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv,
				"cghRl:z:p:t:a:u:A:s:e:v:V:f:r:E:D:n:CZmyxd:W:X:bB:P:S:L:k", opts,
				&index)) != -1) {
		switch (c) {
		case 'c':
